...
ltpoc.ArtifactManager = CFBase.extend({
	init: function() {
	},
		...
		switch (version.toString()) {
			...
			case '1.0':
				...
				if (typeof(archivedArtifact.targetEntityURL) != 'undefined') {
					var eventFact = ltpoc.artifactManager.eventFactFromDescription(fakeDesc);
					if (eventFact) {
						this.eventFactMap[eventFact.url] = eventFact;
					}
				}
				else {
					var entity = ltpoc.artifactManager.entityFromDescription(fakeDesc);
					if (entity) {
			...
			case '1.1':
				...
				var artifact = CFBase.unserialize(archivedArtifact, 'ltpoc.');
	...
	entityFromDescription: function(rdfDescription) {
		...
		if (this.entityMap[url]) {
			...
			return new ltpoc.Entity(rdfDescription);
		}
	},
	...
	eventFactFromDescription: function(rdfDescription) {
		...
		if (this.eventFactMap[url]) {
			...
			return new ltpoc.EventFact(rdfDescription);
		}
	},
	...
	socialTagFromDescription: function(rdfDescription) {
		...
		if (this.socialTagMap[url]) {
			...
			return new ltpoc.SocialTag(rdfDescription);
		}
	},
	...
	docCatFromDescription: function(rdfDescription) {
		...
		if (this.docCatMap[url]) {
			...
			return new ltpoc.DocCat(rdfDescription);
		}
	},
	...
	createArtifactTypeIfNew: function(typeURL) {
		if (typeURL) {
			...
			this.artifactTypes[typeURL] = new ltpoc.ArtifactType(typeURL);
			return this.artifactTypes[typeURL];
		}
		return null;
	},
...
// singleton
ltpoc.artifactManager = new ltpoc.ArtifactManager;
