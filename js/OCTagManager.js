...
ltpoc.TagManager = CFBase.extend({
	init: function() {
	},
	...
	_invokeUpdates: function() {
		...
		ltpoc.updateArchiveField();	// for now, just always call it
		if (this.updateTimerRef) {
	...
	showSuggestedBucket: function() {
		...
		jQuery('#ltpoc_filter_tags_form').slideDown('fast');
	...
	showIgnoredBucket: function() {
		...
		jQuery('#ltpoc_filter_tags_form').slideUp('fast');
	...
	updateFilterList: function() {
		if (!this.deferUpdates) {
			if (this.shouldUpdateFilterList) {
				...
				jQuery('#ltpoc_tag_filter_select').html(optionsHTML);
				if (jQuery.browser.msie && jQuery.browser.version >= 7.0 && jQuery.browser.version < 8.0) {
					if (jQuery('#ltpoc_filter_tags_form').css('display') !== 'none') {
						jQuery('#ltpoc_filter_tags_form').hide().show();	// don't ask.
					}
				}
				if (!currentStillExists) {
					this.filterSuggestedByType('All');
					jQuery('#ltpoc_tag_filter_select option[label=All]').attr('selected', 'selected');
				}
				else {
					this.filterSuggestedByType(this.suggestedFilter);
					jQuery('#ltpoc_tag_filter_select option[label="' + this.suggestedFilter + '"]').attr('selected', 'selected');
				}
			}
		}
		else {
	...
	// use this instead of new ltpoc.Tag
	createTagIfNew: function(text, source) {
		...
		if (typeof(existingTag) === 'undefined') {
			return new ltpoc.Tag(text, source);
		}
...
// singleton
ltpoc.tagManager = new ltpoc.TagManager;
