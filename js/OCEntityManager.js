...
ltpoc.EntityManager = CFBase.extend({
	init: function() {
	},
	...
	unarchiveArtifact: function(archivedArtifact) {
		...
		if (typeof(archivedArtifact.targetEntityURL) !== 'undefined') {
			var eventFact = ltpoc.entityManager.eventFactFromDescription(fakeDesc);
			if (eventFact) {
				this.eventFactMap[eventFact.url] = eventFact;
			}
		}
		else {
			var entity = ltpoc.entityManager.entityFromDescription(fakeDesc);
			if (entity) {
	...
	createArtifactTypeIfNew: function(typeURL) {
		if (typeURL) {
			...
			this.artifactTypes[name] = new ltpoc.ArtifactType(typeURL);
			return this.artifactTypes[name];
		}
		return null;
	},
...
// singleton
ltpoc.entityManager = new ltpoc.EntityManager;
