...
ltpoc.TagToken = CFTextToken.extend({
	init: function(tag) {
		this._super(tag.text, 'ltpoc_tagToken_' + cf.slugify(tag.text + tag.type));
		this.tag = tag;
		var poppet = this;
	...
	getInlineClass: function() {
		if (this.tag.isUserGenerated()) {
			return 'ltpoc_tagToken userInline';
		}
		else {
			var className = this.tag.source.getTagTypeClassName();
			return 'ltpoc_tagToken suggestedInline' + (className.length ? ' ' + className : '');
		}
	},

	getOverlayClass: function() {
		if (this.tag.isUserGenerated()) {
			return 'ltpoc_tagToken userOverlay';
		}
		else {
			var className = this.tag.source.getTagTypeClassName();
			return 'ltpoc_tagToken suggestedOverlay' + (className.length ? ' ' + className : '');
		}
	},
	...
	getContentHTML: function(mode) {
		...
		return killButton + imageButton + addButton + '<span title="' + title + '" id="ltpoc_tagName_' + this.id_tag + '" class="token-text">' + text + '</span>';
	},
	...
	wasInsertedIntoDOM: function() {
		...
		jQuery('#add_textTokenButton_' + this.id_tag + '_overlay').mousedown(function(e) {
			ltpoc.tagManager.putTagInCurrent(poppet.tag);
		});
		jQuery('#kill_textTokenButton_' + this.id_tag + '_overlay').mousedown(function(e) {
			if (!poppet.tag.isUserGenerated()) {
				if (poppet.tag.bucketName === 'current') {
					ltpoc.tagManager.putTagInBlacklist(poppet.tag, 'user');
				}
				else if (poppet.tag.bucketName === 'suggested') {
					ltpoc.tagManager.putTagInBlacklist(poppet.tag, 'user');
				}
				else if (poppet.tag.bucketName === 'blacklisted') {
					ltpoc.tagManager.putTagInSuggested(poppet.tag, 'user');
				}
			}
			else {
				ltpoc.tagManager.deleteTag(poppet.tag);
			}
			cf.gobbleEvent(e);
			return false;
		});
		jQuery('#image_textTokenButton_' + this.id_tag + '_overlay').mousedown(function(e) {
				ltpoc.imageManager.pingFlickr(ltpoc.imageManager.mode_specificTags, true, poppet.tag.text);
				return false;
			}
		);
	...
	overlayClicked: function(e) {
		this._super(e);
		if (this.tag.bucketName !== 'current') {
			ltpoc.tagManager.putTagInCurrent(this.tag);
		}
	},
